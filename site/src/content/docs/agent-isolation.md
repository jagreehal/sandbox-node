...
description: Contain the agent that runs your installs — a host PreToolUse hook that blocks bare npm/npx, or a generated devcontainer that runs the whole session in the jail.
---

sandbox contains installs. This contains the **agent that runs them**. Same policy, two lifecycles.

- **Ephemeral** — the agent works on the host; each dangerous command is jailed the moment it runs. A `PreToolUse` hook enforces it, so the agent literally can't run a bare install. Set up with `sandbox init --agent`.
...
The hook blocks any bare package-manager install/exec or fetch-and-run runner — `npm/pnpm/yarn/bun install · add · run · exec · update · uninstall · …`, bare `yarn`, and `npx/pnpx/bunx` — with a message telling the agent to re-run it as `sandbox <cmd>`. Read-only queries (`npm ls`, `npm view`) stay allowed.
...
Both read `sandbox.config.json`. Change the boundary once — egress, grants, preset — and regenerate the devcontainer. Don't maintain two configs. And keep grants minimal: the `agent` preset keeps host credentials out by default, so add a grant only when the agent genuinely needs it.
