...
`sandbox check` runs the supply-chain gates over what an install *would* pull and reports every finding **without installing anything**. It needs no container — it only queries the registry and the OSV advisory database — so it's the safe first move before you add a dependency.
...
| **Risk hint** (fresh release, exposed bin) | Usually informational — proceed, or pin if it's load-bearing. |
